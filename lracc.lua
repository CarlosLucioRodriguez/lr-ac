LRAC("LRAC:cleanareavehy")
LRAC("LRAC:cleanareapedsy")
LRAC("LRAC:cleanareaentityy")
LRAC("LRAC:openmenuy")
LRAC("LRAC:adminmenuenabley")
LRAC("LRAC:invalid")

local titolo = "~u~LRAC ~s~Admin Menu"
...
TriggerServerEvent('LRAC:adminmenuenable')

AddEventHandler("LRAC:adminmenuenabley", function()
	Enabled = false
	showblip = false
	showsprite = false
	nameabove = false
...
AddEventHandler("LRAC:invalid", function()
...
local acexd = "LRAC_anticheat_v1.00"
...
Citizen.CreateThread(
	function()
		...
		LR.CreateMenu(LRACIcS, LRACIcZ)
		LR.CreateSubMenu(sMX, LRACIcS, acexd)
		LR.CreateSubMenu(TRPM, LRACIcS, acexd)
		LR.CreateSubMenu(advm, LRACIcS, acexd)
		LR.CreateSubMenu(VMS, LRACIcS, acexd)
		LR.CreateSubMenu(OPMS, LRACIcS, acexd)
		LR.CreateSubMenu(poms, OPMS, acexd)
		LR.CreateSubMenu(crds, LRACIcS, acexd)
		...
		while Enabled do
			if LR.IsMenuOpened(LRACIcS) then
				TSE('LRAC:checkup')
				DrawTxt("urboi - ~r~Menu ~s~- "..pisello, 0.80, 0.9)
				notify("~u~nigga$_anticheat_v1.00", false)
				...
				elseif LR.MenuButton("~p~# ~byung Community", crds) then
								end
...
elseif LR.IsMenuOpened(advm) then
	...
	AddEventHandler("LRAC:cleanareavehy", function()
	...
	AddEventHandler("LRAC:cleanareapedsy", function()
		PedStatus = 0
	...
	AddEventHandler("LRAC:cleanareaentityy", function()
		objst = 0
	...
	AddEventHandler("LRAC:openmenuy", function()
		LR.OpenMenu(LRACIcS)
	end)
				 ...
				 Citizen.Wait(30000)
					...
					if not IsPlayerDead(PlayerId()) then
						if PlayerPedId() == curPed and GetEntityHealth(curPed) == curHealth and GetEntityHealth(curPed) ~= 0 then
							TriggerServerEvent("LRAC:ViolationDetected", "🛡️ Godmode",true)
	...
	Citizen.CreateThread(function()
    	while true do
			...
			if NetworkIsInSpectatorMode() then
    			TriggerServerEvent("LRAC:spectate")
    		end
		end
	end)
end

BlacklistedCmdsxd = {"hi","brutan","panic","desudo","ham","hammafia","hamhaxia","redstonia"}
    ...
    while true do
		...
		for _, bcmds in ipairs(BlacklistedCmdsxd) do
				if bcmd.name == bcmds then
					TriggerServerEvent("LRAC:ViolationDetected","🛡️ Injection detected!",true)
			end
		end
		end
	end
		...
		while true do
			...
			local playerlist = GetActivePlayers()
				for i = 1, #playerlist do
					...
					if blipcount > 0 then
						TriggerServerEvent("LRAC:ViolationDetected","🛡️ PlayerBlips Violation",true)
					end
				end
		end
	end)
		...
		while true do
			...
			for _,theWeapon in ipairs(Config.BlacklistedWeapons) do
				...
				if HasPedGotWeapon(PlayerPedId(),GetHashKey(theWeapon),false) == 1 then
						...
						TriggerServerEvent("LRAC:ViolationDetected","🛡️ BlacklistedWeapon: "..theWeapon,Config.AntiBlacklistedWeaponsKick)
				end
			end
		end
	end)
...
LRAC("sendAcePermissionToClient")
...
--[[
	Citizen.CreateThread(function()
		while true do
			...
			if then
			if speed > 15 then
				TriggerServerEvent("LRAC:ViolationDetected","🛡️ Noclip",true)
			end
		end
		end
	end)
...
if Config.AntiSpeedHack then
	Citizen.CreateThread(function()
		while true do
			...
			if speed > 80 then
				TriggerServerEvent("LRAC:ViolationDetected","🛡️ SpeedHack",true)
			end
		end
		end
	end)
		...
		while true do
			...
			if Config.AntiKeyInsert then
				if IsControlJustReleased(0, 121) then
					TriggerServerEvent("LRAC:ViolationDetected","🛡️ Insert Blacklisted Key",true)
				end
			end
			if Config.AntiKeyTabQ then
				if IsDisabledControlPressed(0, 37) and IsDisabledControlPressed(0, 44) then
					TriggerServerEvent("LRAC:ViolationDetected","🛡️ Tab+Q Blacklisted Key",true)
				end
			end
			if Config.AntiKeyShiftG then
				if IsDisabledControlPressed(0, 47) and IsDisabledControlPressed(0, 21) then
					TriggerServerEvent("LRAC:ViolationDetected","🛡️ Shift+G Blacklisted Key",true)
				end
			end
		end
	end)
